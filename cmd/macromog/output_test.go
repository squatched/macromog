...
func TestRun_OutputJSON_BeforeSubcommand(t *testing.T) {
	if got := run([]string{"macromog", "--output", "json", "validate", writeTemp(t, "version: 1\nbooks: {}\n")}); got != 0 {
		t.Errorf("run(--output json validate <valid>) = %d, want 0", got)
	}
}

func TestRun_OutputJSON_AfterSubcommand(t *testing.T) {
...
func TestRun_OutputEqualsSyntax(t *testing.T) {
	if got := run([]string{"macromog", "--output=json", "validate", writeTemp(t, "version: 1\nbooks: {}\n")}); got != 0 {
		t.Errorf("run(--output=json validate <valid>) = %d, want 0", got)
	}
}
